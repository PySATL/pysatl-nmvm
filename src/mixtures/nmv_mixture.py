...
def compute_moment(self, values: list[int], params: dict) -> list[tuple[float, float]]:
    def integral_func(u: float, n: int) -> float:
        result = 0
    ...
    rqmcs = [RQMC(lambda u, n=n: integral_func(u, n), **params) for n in values]
...
def compute_pdf(self, xs: list[float], params: dict) -> list[tuple[float, float]]:
    ...
    for x in xs:
        def inner_func(u: float) -> float:
            ...
            if self.mixture_form == "classical":
                return (
                        ...
                        * np.exp(-((x - self.params.alpha) ** 2 + self.params.beta ** 2 * ppf ** 2) / (
        ...
        rqmc_res = RQMC(inner_func, **params)()
        if self.mixture_form == "classical":
    ...
    for x in xs:
        def inner_func(u: float) -> float:
            ...
            if self.mixture_form == "classical":
                ...
                return -((x - self.params.alpha) ** 2 + ppf ** 2 * self.params.mu ** 2 + ppf * np.log(
                    2 * np.pi * ppf)) / (2 * ppf)

        rqmc_res = LogRQMC(inner_func, **params)()
        if self.mixture_form == "classical":
